...
abstract class FromBaseListener<P extends View, ID> extends RequestListener<ID> {
    ...
    private static final Rect locationParent = new Rect();//容器，比如recyclerview啥的
    private static final Rect locationChild = new Rect();//里面的item

    private final P parentView;
    private final FromTracker<ID> tracker;//用来根据position获取ImageView
